...
import isogleam/tile.{type Tile, type IsoCoord, type TileStatus, IsoCoord, Pending, Generated}
...
pub fn update_tile(grid: Grid, t: Tile) -> Grid {
  ...
  let #(gen_delta, pend_delta) = case old_tile {
    Ok(old) -> {
      let was_generated = case old.status { Generated -> True _ -> False }
      let is_generated = case t.status { Generated -> True _ -> False }
...
pub fn to_json(grid: Grid) -> String {
  let tiles_json = dict.values(grid.tiles)
    |> list.map(fn(t) {
      "{\"x\":" <> int.to_string(t.coord.x)
      <> ",\"y\":" <> int.to_string(t.coord.y)
      <> ",\"status\":\"" <> status_to_string(t.status) <> "\"}"
    })
    |> list.intersperse(",")
    |> list.fold("", fn(acc, s) { acc <> s })

  "{\"width\":" <> int.to_string(grid.width)
  <> ",\"height\":" <> int.to_string(grid.height)
  <> ",\"tiles\":[" <> tiles_json <> "]}"
}
