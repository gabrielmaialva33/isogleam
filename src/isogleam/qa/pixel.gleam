...
/// Image data
pub type ImageData {
  ImageData(width: Int, height: Int, pixels: List(RGB), raw_bytes: BitArray)
}
...
fn get_north_border(img: ImageData, thickness: Int) -> List(RGB) {
  list.flat_map(list.range(0, thickness - 1), fn(y) {
    list.filter_map(list.range(0, img.width - 1), fn(x) { get_pixel(img, x, y) })
  })
}
...
fn get_west_border(img: ImageData, thickness: Int) -> List(RGB) {
  list.flat_map(list.range(0, img.height - 1), fn(y) {
    list.filter_map(list.range(0, thickness - 1), fn(x) { get_pixel(img, x, y) })
  })
}
...
fn do_bytes_to_rgb_graya(bytes: BitArray, acc: List(RGB)) -> List(RGB) {
  case bytes {
    <<g:8, _a:8, rest:bits>> ->
      do_bytes_to_rgb_graya(rest, [RGB(g, g, g), ..acc])
    _ -> acc
  }
}
...
fn do_bytes_to_rgb_rgb(bytes: BitArray, acc: List(RGB)) -> List(RGB) {
  case bytes {
    <<r:8, g:8, b:8, rest:bits>> ->
      do_bytes_to_rgb_rgb(rest, [RGB(r, g, b), ..acc])
    _ -> acc
  }
}
...
fn do_bytes_to_rgb_rgba(bytes: BitArray, acc: List(RGB)) -> List(RGB) {
  case bytes {
    <<r:8, g:8, b:8, _a:8, rest:bits>> ->
      do_bytes_to_rgb_rgba(rest, [RGB(r, g, b), ..acc])
    _ -> acc
  }
}
...
@external(javascript, "../qa/pixel_ffi.mjs", "read_file_sync")
fn read_file(path: String) -> Result(BitArray, String)

@external(erlang, "png_ffi", "decode")
@external(javascript, "../qa/pixel_ffi.mjs", "png_decode")
